...
def preprocess_input_data(inputs):
    data = {
        ...
        'gender': [1 if inputs['gender'] == 'Female' else 0],
...
if st.button('예측하기'):
    ...
    st.sidebar.markdown(f'<h3 style="color: {"red" if prediction[0] >= 0.7 else "blue"}; font-weight: bold;">고객 {"이탈" if prediction[0] >= 0.7 else "유지"}</h3>', unsafe_allow_html=True)
